    ...
    if ball.x_vel<0:
        ...
        if ball.y >= right_paddle.y and ball.y <= right_paddle.y + right_paddle.height:
            if ball.x + ball.radius >= right_paddle.x:
                ball.x_vel *= -1
                middle_y = right_paddle.y + right_paddle.height
                difference_in_y = middle_y - ball.y
...
def main():
    run=True
    left_paddle= Paddle(10,HEIGHT//2-PADDLE_HEIGHT,PADDLE_WIDTH,PADDLE_HEIGHT)
    ...
    while run:
        ...
        if left_score > WINNING_SCORE:
            won = True
            win_text = 'Left player won yayy!!'
        elif right_score > WINNING_SCORE:
            won = True
            win_text='Right player won yayy!!'
        if won:
